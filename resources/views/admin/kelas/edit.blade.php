...
<div class="p-6">
    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
        <form action="{{ route('admin.kelas.update', $kelas->id_kelas) }}" method="POST">
            ...
            <div class="mb-4">
                <label for="id_guru_wali" class="block text-gray-700 font-bold mb-2">Wali Kelas (Opsional)</label>
                <select name="id_guru_wali" id="id_guru_wali" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="">-- Tidak Ada Wali Kelas --</option>
                    @foreach($guruList as $guru)
                        <option value="{{ $guru->id_guru }}" {{ (isset($kelas) && $kelas->id_guru_wali == $guru->id_guru) ? 'selected' : '' }}>
                            {{ $guru->nama_lengkap }}
                        </option>
                    @endforeach
                </select>
